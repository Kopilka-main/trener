...
const ONLINE_COLOR = '#7a8b99';
const GYM_COLOR = '#1a1a1a';
      ...
      {showForm ? (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto pb-6">
            ...
            {view === 'month' && (
              <MonthView anchor={anchor} sessions={sessions} onPickDay={(d) => { setAnchor(d); setView('day'); }} />
            )}
          </div>
        </>
      )}
...
function blockColor(s: Session): string {
  return s.location === 'Онлайн' ? ONLINE_COLOR : GYM_COLOR;
}

// Значок согласования: серый — отправлено клиенту, синий — клиент подтвердил.
function ApprovalBadge({ approval, size = 13 }: { approval: SessionApproval; size?: number }) {
  ...
  return (
    <BadgeCheck
      size={size}
      className="shrink-0"
      style={{ color: approval === 'approved' ? '#2f6fed' : 'var(--color-ink-muted)' }}
      aria-label={approval === 'approved' ? 'Согласовано клиентом' : 'На согласовании'}
    />
  );
}
        ...
        {items.map((s) => {
          ...
          return (
            <button
              key={s.id}
              onClick={() => onPick(s)}
              className="absolute left-1.5 right-1.5 overflow-hidden rounded-xl border-l-[3px] bg-[var(--color-card)] px-2.5 py-1.5 text-left shadow-sm"
              style={{ top, height, borderLeftColor: blockColor(s), opacity: s.status === 'completed' ? 0.5 : 1 }}
            >
                ...
                {items.map((s) => {
                  ...
                  return (
                    <button
                      key={s.id}
                      onClick={() => onPick(s)}
                      className="absolute inset-x-[2px] overflow-hidden rounded-md border-l-2 bg-[var(--color-card)] px-1 py-0.5 text-left"
                      style={{ top, height, borderLeftColor: blockColor(s), opacity: s.status === 'completed' ? 0.5 : 1 }}
                    >
                      <div className="flex items-center gap-0.5">
                        <span className="min-w-0 flex-1 truncate text-[9px] font-bold leading-tight tabular-nums">{s.startTime}</span>
                        <ApprovalBadge approval={s.approval} size={10} />
                      </div>
