...
export function ActiveWorkoutPage() {
  ...
  const finishWorkout = () => {
    navigate(`/workouts/${workout.id}/summary?duration=${elapsed}`);
  };
  ...
  return (
    <div className="flex h-full flex-col">
                      ...
                      {isEditing ? (
                        <SetEditor
                          ...
                          onSave={async (patch) => {
                            ...
                            setEditing(null);
                            if (set.plannedRestSec) setRestSec(set.plannedRestSec);
                          }}
                        />
                      ) : (
      ...
      {restSec !== null && <RestTimer totalSec={restSec} onClose={() => setRestSec(null)} />}
    </div>
  );
}
  ...
  return (
    <span className="inline-flex items-center gap-1">
      ...
      <button
        ...
        className="ml-1 rounded-md bg-ink px-2 py-0.5 text-[11px] font-semibold text-white"
      >
        ✓
      </button>
      <button onClick={onCancel} className="rounded-md px-1.5 py-0.5 text-[11px] text-[var(--color-ink-muted)]">×</button>
    </span>
  );
}
