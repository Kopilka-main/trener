    ...
    <div>
      ...
      {editing && (
        <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
          ...
          <NumField label="отдых" value={row.restSec} onChange={(v) => onChange({ restSec: v ?? 0 })} />
        </div>
      )}
    </div>
  );
...
function ExercisePicker({ open, onClose, onPick }: { open: boolean; onClose: () => void; onPick: (exs: { id: string; name: string; category: string; defaultReps: number | null; defaultWeightKg: number | null; defaultTimeSec: number | null; restSec: number }[]) => void }) {
