...
export function ClientWorkoutsPage() {
  ...
  const onStart = async () => {
    ...
    navigate(`${appBase()}/workouts/${current.id}/active`);
  };

  const onContinue = () => {
    if (current) navigate(`${appBase()}/workouts/${current.id}/active`);
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-6">
        <div className="px-5 pb-3 pt-1 flex items-center gap-3">
          ...
          <button
            onClick={() => navigate(`${appBase()}/calendar?clientId=${id}`)}
...
function estimateDurationMin(w: ClientWorkout): number {
  const totalSec = w.exercises.reduce((acc, ex) => {
    const setDur = ex.sets.reduce((a, s) => a + (s.plannedTimeSec ?? 0) + 30, 0);
    return acc + setDur;
  }, 0);
  return totalSec / 60;
}
